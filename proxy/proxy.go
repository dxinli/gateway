...
// New is new a gateway proxy. 拦截器先于中间件执行
...
// buildMiddleware builds middleware chain. 先进后出 client RoundTripper 为最后一个调用节点
...
// buildEndpoint builds endpoint handler. 先构建中间件链(包括全局中间件和局部中间件) http.RoundTripper
// 然后在构建 http.HandlerFunc 中调用 tripper
